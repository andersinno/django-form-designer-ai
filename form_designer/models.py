...
class FormDefinition(models.Model):
    ...
    body = models.TextField(
        _("body"),
        help_text=_("Form description. Display on form after title."),
        blank=True,
        null=True,
    )
    ...
    mail_cover_text = models.TextField(
        _("email cover text"),
        help_text=_(
            "Email cover text which can be included in the default email template and in the message template."
        ),
        blank=True,
        null=True,
    )
    ...
    mail_from = TemplateCharField(
        _("sender address"),
        max_length=255,
        help_text=MAIL_TEMPLATE_CONTEXT_HELP_TEXT,
        blank=True,
        null=True,
    )
    mail_reply_to = TemplateCharField(
        _("reply-to address"),
        max_length=255,
        help_text=MAIL_TEMPLATE_CONTEXT_HELP_TEXT,
        blank=True,
    ...
    error_message = models.CharField(
        _("error message"), max_length=255, blank=True, null=True
    )
    submit_label = models.CharField(
        _("submit button label"), max_length=255, blank=True, null=True
    )
    ...
    form_template_name = models.CharField(
        _("form template"), max_length=255, blank=True, null=True
    )
    ...
    @warn_about_renamed_method(
        "FormDefinition",
        "string_template_replace",
        "form_designer.utils.string_template_replace",
        DeprecationWarning,
    )
...
class FormDefinitionField(models.Model):
    ...
    widget = models.CharField(
        _("widget"), default="", max_length=255, blank=True, null=True
    )
    ...
    choice_values = models.TextField(
        _("values"), help_text=_("One value per line"), blank=True, null=True
    )
    choice_labels = models.TextField(
        _("labels"), help_text=_("One label per line"), blank=True, null=True
    )
    ...
    regex = RegexpExpressionField(
        _("regular Expression"), max_length=255, blank=True, null=True
    )

    choice_model = ModelNameField(
        _("data model"), max_length=255, blank=True, null=True
    )
    choice_model_empty_label = models.CharField(
        _("empty label"), max_length=255, blank=True, null=True
    )
    ...
    def get_form_field_init_args(self):
        ...
        if self.field_class in (
            ...
            args.update(
                {
                    "queryset": ModelNameField.get_model_from_string(
                        self.choice_model
                    ).objects.all()
                }
            )
        ...
        if self.widget:
            args.update({"widget": import_string(self.widget)()})
...
class FormLog(models.Model):
    form_definition = models.ForeignKey(
        FormDefinition, related_name="logs", on_delete=models.CASCADE
    )
    created = models.DateTimeField(_("Created"), auto_now=True)
    created_by = models.ForeignKey(
        getattr(django_settings, "AUTH_USER_MODEL", "auth.User"),
        null=True,
        blank=True,
...
class FormValue(models.Model):
    form_log = models.ForeignKey(
        FormLog, related_name="values", on_delete=models.CASCADE
    )
